...
def embedding_generator(log: AxiomLogger, secrets: AxiomSecrets, input: EmbeddingRequest) -> EmbeddingVector:
    """Calls the OpenAI embeddings API and returns a dense vector for the input text.
    ...
    if not ok:
        ...
        return EmbeddingVector(values=[], input_text=input.text)
